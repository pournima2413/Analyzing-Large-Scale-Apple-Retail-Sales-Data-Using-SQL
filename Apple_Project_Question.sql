...
--- 14. List the months in the last 4 years where sales exceeded 5000 units from usa.
...
WHERE w.clain_date >= CURRENT_DATE - INTERVAL '3 YEAR'
GROUP BY 1;
...
SELECT
	...
	total_units_sold,
	total_claim,
	COALESCE(total_units_sold::numeric/total_claim::numeric * 100, 0) as risk
FROM
(SELECT 
	st.country,
...
-- 17. Analyze each stores year by year growth ratio for each store.
-- each store and yearly sale
